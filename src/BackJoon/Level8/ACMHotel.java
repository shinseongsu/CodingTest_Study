 ...
 * 고객 설문조사에 따르면 손님들은 호텔 정문으로부터 걸어서 가장 짧은 거리에 있는 방을 선호한다고 한다. 여러분은 지우를 도와 줄 프로그램을 작성하고자 한다. 즉 설문조사 결과 대로 호텔 정문으로부터 걷는 거리가 가장 짧도록 방을 배정하는 프로그램을 작성하고자 한다.
...
public class ACMHotel {
    public static void main(String[] args) {
        ...
        for(int i = 0 ; i < TestCase ; i++) {
            ...
            int W = sc.nextInt();   // 층의 방 수
            int N = sc.nextInt();   // 몇 번 쨰 손님
            int x = 0;
            int y = 0;
